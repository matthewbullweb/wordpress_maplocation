	...
	if ($post->post_type == 'maplocation') {
		switch($hook){
			...
			case 'edit.php':
				...
				wp_enqueue_script( 'geocoder-script', 'https://maps.googleapis.com/maps/api/js?key=<USE OWN KEY>' );
...
function map_front_enqueue() {
	...
	//wp_enqueue_script( 'googlemap-script','https://maps.googleapis.com/maps/api/js?signed_in=false&amp;key=<USE OWN KEY>&amp;callback=initMap' );
	
	//wp_enqueue_script( 'googlemap-script',plugins_url( '/js/googlemaps.js?signed_in=false&amp;key=<USE OWN KEY>&amp;callback=initMap', __FILE__ ) );
